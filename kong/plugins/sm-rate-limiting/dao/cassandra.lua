...
function _M:increment(api_id, identifier, current_timestamp, value)
    ...
    if err then
        ngx_log(ngx_err, "[sm-rate-limiting] could not spawn session to Cassandra: "..tostring(err))
        return
    end
    ...
    for period, period_date in pairs(periods) do
        ...
        if not res then
            ok = false
            ngx_log(ngx_err, "[sm-rate-limiting] could not increment counter for period '"..period.."': ", tostring(err))
        end
    end
