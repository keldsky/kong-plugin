return {
  {
    ...
    down = [[
      DROP TABLE sm_ratelimiting_metrics;
    ]]
  },
  {
    name = "2016-07-25-471385_sm_ratelimiting_policies",
    up = function(_, _, dao)
      local rows, err = dao.plugins:find_all {name = "sm-rate-limiting"}
      if err then return err end

      for i = 1, #rows do
        local sm_rate_limiting = rows[i]

        -- Delete the old one to avoid conflicts when inserting the new one
        local _, err = dao.plugins:delete(sm_rate_limiting)
        if err then return err end

        local _, err = dao.plugins:insert {
          name = "sm-rate-limiting",
          api_id = sm_rate_limiting.api_id,
          consumer_id = sm_rate_limiting.consumer_id,
          enabled = sm_rate_limiting.enabled,
          config = {
            second = sm_rate_limiting.config.second,
            minute = sm_rate_limiting.config.minute,
            hour = sm_rate_limiting.config.hour,
            day = sm_rate_limiting.config.day,
            month = sm_rate_limiting.config.month,
            year = sm_rate_limiting.config.year,
            limit_by = "consumer",
            policy = "cluster",
            fault_tolerant = sm_rate_limiting.config.continue_on_error
          }
        }
        if err then return err end
      end
