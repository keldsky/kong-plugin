...
```bash
for n in $(curl https://consul.stg.pdx.intsm.net/v1/catalog/nodes | jq '.' | grep "kong" | grep -v "cassandra" | awk {'print $2'} | sed 's/\"//g' | sed 's/,//g'); do yes | ssh $n.stg.pdx.intsm.net "sudo apt-get update; yes | sudo apt-get install kong-plugin"; done
```
